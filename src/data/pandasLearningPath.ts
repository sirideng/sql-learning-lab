...
function buildChapter(spec: ChapterSpec, index: number): PandasChapter {
  return {
    ...
    mistakes: spec.pitfalls.slice(0, 2).map(([title, problem, fix]) => ({ title, problem, fix })),
...
const specs: ChapterSpec[] = [
  {
    ...
    pitfalls: [['把索引当作 user_id', '默认索引只是行标签，不保证稳定，也不等于业务主键。', "使用 people['user_id'] 读取明确的业务键。"], ['混淆一层与两层中括号', "df['age'] 返回 Series，而 df[['age']] 返回保留二维结构的 DataFrame。", "需要二维结果时使用 people[['age']]。"]],
  ...
  },
  {
    ...
    pitfalls: [['只看 head 就判断分布', 'head 只展示开头几行，不能代表完整分布、异常值或缺失情况。', '结合 people.sample(3)、people.describe() 和 people.isna().sum()。'], ['链式选择后误修改原表', '切片可能是视图也可能是副本，直接赋值容易产生不确定行为。', "使用 subset = people.loc[:, ['user_id', 'city']].copy() 后再修改。"]], exercises: [['随机查看3行用什么？', '使用 sample。', 'people.sample(3)'], ['查看字段类型用什么？', '使用 dtypes 或 info。', 'people.dtypes'], ['为什么不建议分析全程保留全部列？', '考虑可读性和性能。', '无关字段增加认知负担与内存占用。']], checklist: ['会预览数据', '会选择多列', '会检查类型', '能描述结果粒度'],
  },
  {
    ...
    pitfalls: [['用 and 连接 Series', 'and 只接受单个布尔值，无法逐行组合两个布尔 Series。', "使用 (people['city'] == '上海') & (people['age'] >= 20)。"], ['混淆 loc 与 iloc', 'loc 按标签选择，iloc 按整数位置选择，混用会取到错误的行列。', "按标签使用 people.loc[:, ['user_id', 'age']]；按位置使用 people.iloc[:2, :2]。"]], exercises: [['筛选 age>20 怎么写？', '构造布尔 Series。', "people.loc[people['age'] > 20]"], ['取前两行前两列怎么写？', '使用 iloc。', 'people.iloc[:2, :2]'], ['多个条件为什么要括号？', '& 优先级容易产生错误。', "(cond1) & (cond2)"]], checklist: ['会构造布尔掩码', '会组合多个条件', '能区分 loc/iloc', '知道筛选后何时 copy'],
  },
  {
    ...
    pitfalls: [['错误覆盖原始字段', '直接覆盖 amount 会丢失原始口径，后续无法核对计算。', "先使用 orders.assign(tax_amount=orders['amount'] * 1.06) 创建新列。"], ['赋值 Series 索引不一致', 'Pandas 按索引标签对齐，标签不同会产生错位或 NaN。', "先用 values = values.reindex(orders.index)，再执行 orders['new_value'] = values。"]], exercises: [['新增 amount 的平方列？', '直接进行向量化。', "orders.assign(amount_sq=orders['amount'] ** 2)"], ['重命名 amount 为 revenue？', '使用 rename。', "orders.rename(columns={'amount':'revenue'})"], ['为什么优先向量化？', '避免逐行 Python 循环。', '更快、更简洁且语义清楚。']], checklist: ['会新增列', '会修改和重命名列', '会安全删除列', '理解索引对齐'],
  },
  {
    ...
    pitfalls: [['把所有缺失填0', '缺失表示未知，0 可能是有效观测，混用会改变均值和分布。', "确认业务口径后使用 scores['score'].fillna(scores['score'].median())。"], ['dropna 后不检查样本量', '删除缺失行可能让某些群体被系统性排除，造成样本偏差。', '先记录 before = len(scores)，再比较 len(scores.dropna(subset=[\'score\']))。']], exercises: [['统计每列缺失数？', 'isna 后 sum。', 'scores.isna().sum()'], ['用中位数填分数？', '先计算 median。', "scores['score'].fillna(scores['score'].median())"], ['缺失为什么不等于0？', '0可能是有效观测。', '缺失表示未知，0表示已知值。']], checklist: ['会识别缺失', '能选择填补策略', '会评估删除影响', '区分缺失与业务零值'],
  },
  {
    ...
    pitfalls: [['没有 subset 就去重', '整行去重无法识别同一业务事件的不同版本，可能继续重复计数。', "使用 events.drop_duplicates(subset=['event_id']) 明确业务唯一键。"], ['字符串日期直接排序', '不同日期格式按字符排序时不保证时间先后正确。', "先执行 events['date'] = pd.to_datetime(events['date'])，再 sort_values('date')。"]], exercises: [['按 amount 降序？', 'ascending=False。', "orders.sort_values('amount', ascending=False)"], ['按两列去重？', 'subset 接收列表。', "df.drop_duplicates(['user_id','date'])"], ['为什么 shift 前必须排序？', '上一行取决于当前顺序。', '先按实体键和时间排序。']], checklist: ['会多列排序', '会按业务键去重', '会转换日期类型', '理解排序对行间运算的影响'],
  },
  {
    ...
    pitfalls: [['用 count 统计订单行数', 'count 会跳过缺失值，字段有空值时不等于组内总行数。', "使用 orders.groupby('user_id').size()，或对非空主键执行 count。"], ['忘记结果粒度变化', 'groupby 聚合会把多条明细压缩为每组一行，不能再假设结果仍是订单粒度。', "使用 orders.groupby('user_id', as_index=False).agg(total_amount=('amount', 'sum')) 明确用户粒度。"]], exercises: [['按城市算平均年龄？', 'groupby city 后 mean。', "people.groupby('city',as_index=False).agg(avg_age=('age','mean'))"], ['size 和 count 区别？', 'count 跳过缺失。', 'size 数行，count 数非空。'], ['为什么 as_index=False？', '让分组键保留为普通列。', '便于继续 merge 和展示。']], checklist: ['理解 split-apply-combine', '会命名聚合', '能控制结果索引', '会解释聚合粒度'],
  },
  {
  ...
  },
  {
    ...
    pitfalls: [['直接对字符串日期做减法', 'object 字符串没有时间间隔语义，不能可靠计算天数。', "先执行 df['date'] = pd.to_datetime(df['date'])，再进行日期相减。"], ['忽略时区', '同一时刻在不同时区可能属于不同自然日，直接提取日期会错分活跃日。', "先使用 events['time'].dt.tz_convert('Asia/Shanghai') 统一业务时区。"]], exercises: [['提取星期几？', '使用 dt.day_name。', "df['date'].dt.day_name()"], ['计算两个日期天数？', '相减后 dt.days。', "(df['end']-df['start']).dt.days"], ['Period 与 datetime 区别？', '一个表示周期，一个表示时间点。', '月度聚合常用 Period。']], checklist: ['会解析日期', '会提取日期部件', '会计算 Timedelta', '理解留存观察窗口'],
  },
  {
    ...
    pitfalls: [['直接使用 Python 字符串方法', 'Series 不是单个字符串，调用 .lower() 无法逐行处理整列。', "使用 accounts['email'].str.strip().str.lower()。"], ['正则特殊字符未转义', 'str.replace 默认的正则语义可能把特殊字符解释为模式而非字面量。', "字面替换使用 s.str.replace('-', '', regex=False)。"]], exercises: [['取手机号前3位？', 'str.slice。', "df['phone'].str[:3]"], ['替换连字符？', 'str.replace。', "s.str.replace('-','',regex=False)"], ['为什么先清洗再 groupby？', '避免同类被分开。', '标准化后类别键才稳定。']], checklist: ['会标准化文本', '会提取子串', '会构造文本筛选', '理解清洗对分组的影响'],
  },
  {
    ...
    pitfalls: [['能向量化却使用 axis=1 apply', '逐行 Python 函数更慢，也更难表达缺失值和类型规则。', "优先使用 employees['salary'] * 1.1 或 np.select(...) 等向量化写法。"], ['用 agg 期望保持原行数', 'agg 返回每组一行，无法与原始明细逐行对齐。', "需要广播时使用 employees.groupby('department')['salary'].transform('mean')。"]], exercises: [['组内最大值标回原行？', 'transform max。', "df.groupby('group')['value'].transform('max')"], ['apply axis=1 表示什么？', '逐行传入 Series。', '每次函数接收一行。'], ['transform 与窗口函数关系？', '都保留原粒度。', 'SQL AVG OVER 对应 groupby.transform(mean)。']], checklist: ['会区分 apply/agg/transform', '能优先向量化', '理解广播回原行', '会对照 SQL 窗口函数'],
  },
  {
    ...
    pitfalls: [['未排序就 shift', 'shift 只参考当前行顺序，未按时间排序时“上一行”不等于“上一期”。', "先执行 ordered = daily.sort_values('date')，再 ordered['previous'] = ordered['value'].shift(1)。"], ['多用户数据未 groupby', '直接 shift 会把上一位用户的最后一行移动到下一位用户。', "使用 df.groupby('user_id')['value'].shift(1) 在用户内移动。"]], exercises: [['计算增长率？', 'pct_change。', "df['value'].pct_change()"], ['每用户累计金额？', 'groupby 后 cumsum。', "df.groupby('user_id')['amount'].cumsum()"], ['diff 为什么首行 NaN？', '没有上一行可相减。', '首行缺少前一期基准。']], checklist: ['理解 shift 对齐', '能手算 diff', '会组内累计', '会选择排名规则'],
  },
  {
    ...
    pitfalls: [['pivot 遇到重复键报错', 'pivot 要求每个行键与列键组合唯一，重复组合无法确定单个单元格值。', "有重复时使用 sales.pivot_table(index='city', columns='product', values='amount', aggfunc='sum')。"], ['透视后多层索引难用', '索引键或多层列不再是普通字段，会影响后续 merge 和导出。', '执行 result = wide.reset_index()，并按需要整理 result.columns。']], exercises: [['宽转长使用什么？', 'melt。', 'df.melt(id_vars=...)'], ['pivot 与 pivot_table 区别？', '后者支持聚合。', '重复键时用 pivot_table。'], ['透视结果为什么常 reset_index？', '让索引键恢复普通列。', '便于 merge 和导出。']], checklist: ['会判断长宽结构', '会选择 pivot/pivot_table', '会使用 melt', '会处理透视后的索引'],
  },
  {
    ...
    pitfalls: [['清洗前不保留原始数据', '原地覆盖会失去复核依据，出现异常时无法定位是哪一步改变了数据。', '先执行 clean = raw.copy()，后续只修改 clean。'], ['同时做太多转换', '去重、填补和文本清洗混在一步时，行数或数值变化难以归因。', "按步骤执行并记录 len(clean)，例如先 clean = clean.drop_duplicates('order_id')。"]], exercises: [['检查重复订单数？', 'duplicated 后 sum。', "raw.duplicated('order_id').sum()"], ['验证 order_id 唯一？', 'nunique 与行数比较。', "clean['order_id'].nunique() == len(clean)"], ['为什么记录行数变化？', '发现意外丢行或扩行。', '每步记录 before/after rows。']], checklist: ['会先审计再清洗', '会分步转换', '会记录行数变化', '会编写质量断言'],
  },
  {
    ...
    pitfalls: [['需求没有明确口径', '分子、分母、时间范围或粒度不清时，同一个指标会得到多个都“像对的”答案。', "先写下口径，例如 paid.query(\"amount > 0\") 后用 paid['amount'].sum() 计算 GMV。"], ['只检查最终表', '只看最终数字无法发现中间 JOIN 扩行、过滤丢行或键重复。', "每步检查 df.shape、df['user_id'].nunique() 和 df.isna().sum()。"]], exercises: [['客单价如何定义？', '先明确分母是订单。', 'GMV / 有效订单数'], ['复购率分母是什么？', '所有购买用户。', '订单数>=2用户 / 购买用户'], ['怎样交付可复核结果？', '保留代码、输入版本和质量检查。', '分析流程、指标表、结论与限制一起交付。']], checklist: ['能定义业务指标', '会构建分层 DataFrame', '会验证中间结果', '能解释结论与限制'],
  },
]
