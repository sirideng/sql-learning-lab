...
export function LearningPathPage({ chapters, completedLessons, completedProblems, totalProblems, initialChapter, onNavigateSection, onToggleLesson, onOpenPractice }: Props) {
  ...
  const select = (id: string) => { setSelectedId(id); window.location.assign(`#/learn/${id}`); window.scrollTo({top:0,behavior:'smooth'}) }
  const isComplete = completedLessons.includes(chapter.id)
  return <div className="learning-page atlas-page"><AppHeader completed={completedProblems} total={totalProblems} currentSection="learn" onHome={()=>onNavigateSection('dashboard')} onNavigateSection={onNavigateSection}/><main className="atlas-main learning-map-page">
    <header className="page-title"><span className="eyebrow">LEARNING MAP</span><h1>数据分析学习地图</h1><p>先选模块，再进入短而完整的一节课。原章节链接和学习进度继续保留。</p><div className="language-local-switch"><button className="active">SQL</button><button onClick={()=>window.location.assign('#/pandas/learn')}>Pandas</button></div></header>
    <section className="module-map" aria-label="SQL 学习模块">{modules.map((module,index)=><button key={module.title} className={index===activeModule?'active':''} onClick={()=>select(module.chapters[0].id)}><span>{String(index+1).padStart(2,'0')}</span><strong>{module.title}</strong><small>{module.chapters.filter(c=>completedLessons.includes(c.id)).length}/{module.chapters.length}</small></button>)}</section>
    <div className="lesson-workspace"><aside className="lesson-subnav"><strong>{modules[activeModule]?.title}</strong>{modules[activeModule]?.chapters.map(item=><button key={item.id} className={item.id===chapter.id?'active':''} onClick={()=>select(item.id)}>{completedLessons.includes(item.id)?<CheckCircle2 size={17}/>:<span>{String(item.order).padStart(2,'0')}</span>}<div><b>{item.title}</b><small>{item.subtitle}</small></div></button>)}</aside>
      <article className="compact-lesson" key={chapter.id}><header><span>CHAPTER {String(chapter.order).padStart(2,'0')}</span><h2>{chapter.title}</h2><p>{chapter.description}</p></header>
