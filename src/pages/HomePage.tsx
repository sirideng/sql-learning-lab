  ...
  return (
    <div className="home-page">
      ...
      <main className="home-main">
        ...
        <section className={`library-section ${libraryOnly ? 'library-only' : ''}`}>
          <div className="section-heading">
            <div>
              <span className="eyebrow">PRACTICE LAB</span>
              <h2>{libraryOnly ? 'SQL 数据分析练习' : '题库'}</h2>
              {libraryOnly && <p className="section-description">20 道题，从基础查询逐步走向用户与商业数据分析。</p>}
            </div>
            <label className="search-box">
              <Search size={17} />
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索题目或标签" />
            </label>
          </div>
          <div className="library-controls"><div className="filter-row">
            ...
            <select aria-label="按难度筛选" value={difficulty} onChange={(event) => setDifficulty(event.target.value as (typeof difficultyFilters)[number])}>
              {difficultyFilters.map((item) => <option key={item}>{item}</option>)}
            </select>
            <select aria-label="按章节筛选" value={chapter} onChange={(event) => setChapter(event.target.value)}>{chapters.map((item) => <option key={item}>{item}</option>)}</select>
          </div></div>
