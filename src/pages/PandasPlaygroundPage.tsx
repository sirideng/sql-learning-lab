...
export function PandasPlaygroundPage({ scenarios, completed, total, onNavigateSection }: Props) {
  ...
  return <div className="pandas-playground-page">
    ...
    <main className="pandas-playground-main">
      <section className="playground-heading"><div><span className="eyebrow">PRACTICE SANDBOX / LEARNING SIMULATOR</span><h1>Pandas Playground</h1><p>在预定义数据集上练习核心 DataFrame 操作，并逐步观察每次转换。</p></div><label>实验场景<select value={selectedId} onChange={(event) => { setSelectedId(event.target.value); setStatus(null) }}>{scenarios.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></section>
