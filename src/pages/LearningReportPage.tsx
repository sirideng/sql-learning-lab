...
import type { ActivityMap, PandasQuestion, ProgressMap, SqlProblem } from '../types/problem'

interface Props { problems: SqlProblem[]; pandasProblems: PandasQuestion[]; progress: ProgressMap; activity: ActivityMap; completedLessons: string[]; onNavigateSection: (section: AppSection) => void }
...
export function LearningReportPage({ problems, pandasProblems, progress, activity, completedLessons, onNavigateSection }: Props) {
  const all = [...problems, ...pandasProblems]
  ...
  return <div className="home-page atlas-page"><AppHeader completed={completed} total={all.length} currentSection="report" onHome={() => onNavigateSection('dashboard')} onNavigateSection={onNavigateSection} /><main className="home-main atlas-main report-page">
    ...
    {Object.keys(activity).length > 0 ? <ActivityHeatmap activity={activity} /> : <Empty title="还没有做题记录" text="完成一次练习后，这里会出现最近半年的活动热力图。" action="去练习" onClick={() => onNavigateSection('practice')} />}
